...
*/
__global__ void bev_pool_kernel(int64_t b, int64_t d, int64_t h, int64_t w, int64_t n, int64_t c, int64_t n_intervals,
                                  const float *__restrict__ x,
                                  const int64_t *__restrict__ geom_feats,
                                  const int64_t *__restrict__ interval_starts,
                                  const int64_t *__restrict__ interval_lengths,
                                  float* __restrict__ out) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  int64_t index = idx / c;
  int64_t cur_c = idx % c;
  if (index >= n_intervals) return;
  int64_t interval_start = interval_starts[index];
  int64_t interval_length = interval_lengths[index];
  const int64_t* cur_geom_feats = geom_feats + interval_start * 4;
...
void bev_pool(int64_t b, int64_t d, int64_t h, int64_t w, int64_t n, int64_t c, int64_t n_intervals, const float* x,
  const int64_t* geom_feats, const int64_t* interval_starts, const int64_t* interval_lengths, float* out) {
  bev_pool_kernel<<<(int64_t)ceil(((double)n_intervals * c / 256)), 256>>>(
