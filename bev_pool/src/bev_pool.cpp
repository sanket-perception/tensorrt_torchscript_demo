...
at::Tensor bev_pool_forward(
  ...
  int64_t n = _x.size(0);
  int64_t c = _x.size(1);
  int64_t n_intervals = _interval_lengths.size(0);
  const at::cuda::OptionalCUDAGuard device_guard(device_of(_x));
  const float* x = _x.data_ptr<float>();
  const int64_t* geom_feats = _geom_feats.data_ptr<int64_t>();
  const int64_t* interval_lengths = _interval_lengths.data_ptr<int64_t>();
  const int64_t* interval_starts = _interval_starts.data_ptr<int64_t>();
...
//   torch::RegisterOperators("mynamespace::bev_pool_forward", &bev_pool_forward);
